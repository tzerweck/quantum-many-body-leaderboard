...
// Below the frontier table every instance is listed, one collapsed table per model. Those
// labels are built from the instance itself, so anything added to data/ appears without a
// hand-written entry.
export const MODELS = [
...
function main() {
  ...
  for (const [id, label] of FRONTIER) {
    ...
    lines.push(row(inst, label, cache));
  }
  ...
  lines.push("");
  lines.push(`### All ${s.instances} instances`);
  lines.push("");
  ...
  console.log(`README.md leaderboard: ${FRONTIER.length} frontier + ${s.instances} by model, ${s.records.held}/${s.instances} records held`);
}
