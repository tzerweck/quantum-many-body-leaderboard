...
if [ "${1:-}" = "--fetch" ]; then
  $SSH 'cd ~/agent-runs/qmbl-cost && tar czf - results euler/logs' | tar xzf - -C .
  ls -la results; exit 0
fi
git rev-parse --short HEAD > COMMIT
tar czf - --exclude=results --exclude=euler/logs . | $SSH 'mkdir -p ~/agent-runs/qmbl-cost && cd ~/agent-runs/qmbl-cost && tar xzf - && echo synced && ls'
