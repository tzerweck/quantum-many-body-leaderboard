...
has a 4d hidden layer; the token mean z in R^d; and two real hidden layers of log cosh
units, one for the log-amplitude and one for the phase, log psi = A(z) + i P(z), so that
every parameter is real. The head starts small (kernels at 0.01 / sqrt(d)) so the initial
state is close to uniform and sampleable: with a token sum and 0.05 kernels the initial
log-amplitudes spread over hundreds of nats and the first energy read 2e5 (smoke job).
...
class ViT(nn.Module):
    ...
    def __call__(self, sigma):  # sigma: (..., L*L) in {-1, +1}
        ...
        z = nn.LayerNorm(name="final_norm")(x).mean(axis=-2)  # (..., d)
        head = nn.initializers.normal(0.01 / np.sqrt(self.d))
        amp = nk.nn.log_cosh(nn.Dense(self.d, name="amplitude", kernel_init=head)(z)).sum(axis=-1)
        phase = nk.nn.log_cosh(nn.Dense(self.d, name="phase", kernel_init=head)(z)).sum(axis=-1)
        return amp + 1j * phase
